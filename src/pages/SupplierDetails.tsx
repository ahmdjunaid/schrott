    ...
    <div id="printable-invoice" className="space-y-8 py-4 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 print:hidden">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-3xl font-black text-slate-900 tracking-tighter">Supplier Ledger</h2>
            <p className="text-sm font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2 mt-1">
               <Calendar size={14} /> Purchases & Settlement History for {supplier?.shop_name}
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="md:col-span-2 bg-white p-8 rounded-3xl border border-slate-200 shadow-xl shadow-slate-200/20 flex items-center gap-6 group">
           <Avatar fallback={supplier?.shop_name || 'SP'} size="xl" className="shadow-2xl shadow-slate-200 group-hover:scale-105 transition-transform" />
           <div>
              <h3 className="text-2xl font-black text-slate-900 tracking-tight leading-none">{supplier?.shop_name}</h3>
              <div className="flex flex-col gap-2 mt-3 text-xs font-bold text-slate-400 uppercase tracking-widest">
                 <div className="flex items-center gap-2"><MapPin size={12} className="text-primary" /> {supplier?.location || 'Regional Terminal'}</div>
        ...
        <div className="bg-rose-50 p-8 rounded-3xl border border-rose-100 shadow-xl shadow-rose-500/5 relative overflow-hidden group print:hidden">
           ...
           <div className="relative">
              ...
              <div className="text-3xl font-black text-rose-600 italic tracking-tighter leading-tight">₹{supplier?.balance?.toFixed(2) || '0.00'}</div>
           </div>
        </div>
        ...
        <div className="bg-emerald-50 p-8 rounded-3xl border border-emerald-100 shadow-xl shadow-emerald-500/5 relative overflow-hidden group print:hidden">
           ...
           <div className="relative">
              ...
              <div className="text-3xl font-black text-emerald-600 italic tracking-tighter leading-tight">₹{supplier?.wallet_balance?.toFixed(2) || '0.00'}</div>
           </div>
        </div>
        ...
        <div className="hidden print:block col-span-4 space-y-10">
           <div className="border-l-4 border-rose-600 pl-6 py-2">
              ...
              <div className="text-5xl font-black text-slate-900">₹{supplier?.balance?.toFixed(2) || '0.00'}</div>
           </div>
           <div className="border-l-4 border-emerald-600 pl-6 py-2">
              ...
              <div className="text-5xl font-black text-slate-900">₹{supplier?.wallet_balance?.toFixed(2) || '0.00'}</div>
           </div>
        </div>
      </div>
