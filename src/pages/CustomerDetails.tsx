  ...
  const fetchLedger = async () => {
    try {
      ...
      const [cData, tData] = await Promise.all([
        ...
        customerService.getTransactions(id!, startDate)
      ]);
    ...
    <div className="space-y-4 md:space-y-8 py-2 md:py-4 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 md:gap-6 print:hidden">
        <div className="flex items-center gap-3 md:gap-4">
          ...
          <div>
            <h2 className="text-xl md:text-3xl font-black text-slate-900 tracking-tighter uppercase md:normal-case italic">Customer Ledger</h2>
            <p className="text-[9px] md:text-sm font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2 mt-0.5 md:mt-1">
               <Calendar size={12} /> STATEMENT: {customer?.shop_name}
            </p>
          </div>
        </div>
      ...
      <div className="flex flex-col lg:flex-row gap-4 md:gap-6">
        <div className="flex-1 bg-white p-5 md:p-8 rounded-2xl md:rounded-3xl border border-slate-200 shadow-xl shadow-slate-200/20 flex items-center gap-4 md:gap-6 group">
           <Avatar fallback={customer?.shop_name || 'CL'} size="lg" className="shadow-2xl shadow-slate-200 group-hover:scale-105 transition-transform" />
           <div>
              <h3 className="text-xl md:text-2xl font-black text-slate-900 tracking-tight leading-none italic uppercase">{customer?.shop_name}</h3>
              <div className="flex flex-col gap-1.5 md:gap-2 mt-2 md:mt-3 text-[10px] md:text-xs font-bold text-slate-400 uppercase tracking-widest">
                 <div className="flex items-center gap-2"><MapPin size={12} className="text-primary" /> {customer?.location || 'Registered Store'}</div>
              ...
              <div className={cn(
                ...
                customer?.balance > 0 ? "text-rose-500" : "text-emerald-500"
                 ...
                 <span className="text-[10px] ml-2 opacity-50 uppercase tracking-widest font-bold">
                    {customer?.balance > 0 ? 'To Receive' : 'Advance'}
                 </span>
              </div>
           </div>
        </div>
                ...
                <td className="px-8 py-5">
                   <div>
                      <div className={cn(
                        ...
                        t.type === 'RETURN' ? 'text-indigo-600' : 
                        t.type === 'REFUND' ? 'text-rose-500' : 'text-emerald-600'
                      )}>
                         {t.type === 'INVOICE' ? 'SALE / BILL' : 
                ...
                <td className="px-8 py-5">
                    {t.type === 'PAYMENT' ? (
                      <div className="font-black text-emerald-600 text-sm italic">
                       ...
                       <div className="font-black text-indigo-600 text-sm italic">
                   ...
                   <div className={cn(
                     ...
                     t.running_balance > 0 ? "text-rose-600" : "text-emerald-600"
                      ...
                      <span className="text-[8px] ml-2 opacity-50 uppercase tracking-tighter">
                        {t.running_balance > 0 ? 'Due' : 'Advance'}
                      </span>
                   </div>
                </td>
              </tr>
            ...
            {transactions.length === 0 && (
              <tr>
                <td colSpan={5} className="px-10 py-32 text-center">
                   ...
                   <p className="text-slate-400 font-bold uppercase tracking-widest text-[10px]">No ledger entries discovered</p>
                </td>
              </tr>
            )}
          </Table>
