-- FINAL FIX: Ensuring Purchase RPC matches the GST schema and Wallet logic
...
BEGIN
  -- 1. Calculate the real total from all items (Price + SGST + CGST)
  ...
  IF v_wallet_credit > 0 THEN
    -- Update supplier wallet: subtract used amount (capped at purchase total)
    UPDATE suppliers 
  ...
  -- 4. Create the main Purchase record
  ...
  -- Trace for manual payment (Cash, UPI, etc.)
  ...
  LOOP
    ...
    VALUES (
      ...
      (v_item->>'quantity')::INTEGER, -- remaining_qty matches quantity initially
